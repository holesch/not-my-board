...
class Hub:
    ...
    def __init__(self, config=None, http_client=None):
        ...
        if auth_info:
            ...
            trusted_issuers = {auth_info["issuer"]}
            ...
            def make_permission(d):
                d["claims"].setdefault("iss", auth_info["issuer"])
                return Permission.from_dict(d)

            self._permissions = [make_permission(d) for d in config["permissions"]]
            self._validator = auth.Validator(
                auth_info["client_id"], http_client, trusted_issuers
            )
        else:
...
class Authenticator(util.ContextStack):
    ...
    async def _request_roles(self):
        ...
        token_claims = await self._validator.extract_claims(id_token)
        roles = set()
        ...
        for permission in self._permissions:
            ...
            for key, needed_claim in permission.claims.items():
                token_claim = token_claims.get(key)
                if isinstance(needed_claim, set):
                    if (
                        not isinstance(token_claim, list)
                        or set(token_claim) < needed_claim
                    ):
                        break
                else:
                    if token_claim != needed_claim:
                        break
            else:
